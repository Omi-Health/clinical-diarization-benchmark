...
def test_best_settings_log_matches_published_rows():
    ...
    assert {'sortformer1', 'sortformer21', 'sortformer21_low_unpaced', 'community1'} <= set(receipt['settings_log'])
    assert not [k for k in receipt['settings_log'] if k.startswith('model_x')]
    ...
    for key, s in speed.items():
        assert s['kind'] in {'l4_batch', 'api_round_trip', 'joint_server', 'paced_live'}
        if s['kind'] == 'paced_live':
            assert 'median_s_per_file' not in s and 'note' in s
            continue
        assert s['median_s_per_file'] > 0
        if s['kind'] == 'l4_batch':
