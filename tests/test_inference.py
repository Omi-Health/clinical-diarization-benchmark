...
def test_geometry_is_validated_and_nemotron3_presets_are_ready():
    class Modules:
        chunk_len = 1
    class Model:
        streaming_mode = True
    ...
    assert {p.stem for p in configs.glob('*.json')} == {'sortformer1', 'sortformer21', 'sortformer21_low_unpaced', 'nemotron3', 'nemotron3_streaming_unpaced'}

    for key in ['nemotron3', 'nemotron3_streaming_unpaced']:
        config = json.loads((configs / (key + '.json')).read_text())
        assert config['model_id'] == 'nvidia/Nemotron-3-Diarization'
        assert config['geometry']
        assert len(config['checkpoint_sha256']) == 64 and len(config['model_revision']) == 40
    streaming = json.loads((configs/'nemotron3_streaming_unpaced.json').read_text())
...
def test_model_name_alone_resolves_checkpoint_and_records_revision(tmp_path):
    ...
    config = json.loads((Path(__file__).resolve().parents[1]/'inference/configs/nemotron3.json').read_text())
    config['model_id'] = 'example/diarization'
    for key in ('model_revision', 'checkpoint_sha256', 'checkpoint_file'): config.pop(key, None)
