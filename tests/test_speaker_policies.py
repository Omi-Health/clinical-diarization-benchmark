...
def test_paired_snapshot_is_matched_and_model_x_stays_private():
    ...
    for m in snapshot['models']:
        ...
        if m['key'].startswith('model_x'):
            assert not m['public_outputs'] and 'source' not in m
            for panels in m['policies'].values():
                for bodies in panels.values():
                    for body in bodies.values():assert set(body)=={'aggregate'}
        else:
