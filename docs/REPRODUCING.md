...
For the NVIDIA model rows, use [the frozen baseline instructions](GA_20260924.md#reproduce) and `python scripts/verify_ga.py`. The commands below reproduce the scoring of every saved output.
...
│   ├── snapshot.json         # Scores and per-case counts for every saved output
│   ├── ga-20260924/          # L4 run of the NVIDIA models: scores, receipts, crosschecks
...
This release reproduces **scoring from saved outputs**, rather than rerunning every model. Standalone [inference adapters](../inference/README.md) are included for Sortformer and Nemotron 3 Diarization. Weights and live API access are not bundled; the Nemotron 3 preset pins the released checkpoint by revision and SHA-256.

The public scoring workflow needs only the files in this repository. Nemotron 3 inference uses the pinned public model ID.
