...
Inference has not been rerun for this public release. Standalone inference adapters for Sortformer and configurable Model X runs are included; see [inference code](../inference/README.md). They were extracted for review and have not been freshly rerun on a GPU. Other baseline inference clients and a fully pinned end-to-end environment are not bundled. Checkpoint revisions and full inference settings are not uniformly captured in the public snapshot. The displayed names identify the historical runs; they are not assertions about the latest versions of those products.
